...
<div class="container-fluid">
    <div class="fade-in">
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-header"><i class="fa fa-align-justify"></i> Press Release</div>
                <div class="card-body">
                    <table class="table table-responsive-sm table-bordered table-striped table-sm">
                        ...
                        <tbody >
                            ...
                            <tr data-id="{{$pressRelease->id}}">
                                <td>{{$key+1}}</td>
                                <td>{{ $pressRelease->title }}</td>
                                <td>{{ $pressRelease->date }}</td>
                                <td>{!! $pressRelease->description !!}</td>
                                <td>
                                    <img src="{{Storage::url($pressRelease->image)}}" height="100">
                                </td>

                                <td>{!! $pressRelease->publish?'<span
                                        class="badge badge-pill badge-success">Active</span>':'<span
                                        class="badge badge-pill badge-warning">Inactive</span>' !!}</td>
                                <td>
                                    <button class="btn btn-xs btn-info"
                                    onclick="window.location=`{{ route('PressRelease.edit',['id'=>$pressRelease->id]) }}`">Edit</button>
                                    <button data-question="Are you sure to delete the data?" data-toggle="confirm" data-id="{{ $pressRelease->id }}" class="btn btn-xs btn-danger">Delete</button>
                                </td>
                            </tr>
                            @empty
                            <tr>
...
<script>
    $(function(){
        $('[data-toggle="confirm"]').jConfirm().on('confirm', function(e){
            var btn = $(this),
            id = btn.data('id');
            var url = '{{ route("PressRelease.delete", ":id") }}';
