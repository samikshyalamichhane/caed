...
class AnnouncementController extends Controller
{
    ...
    {
        $announcements = Announcement::orderBy('created_at', 'ASC')->paginate(100);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $announcement->publish = $request->publish ? 1 : 0;
            if ($request->hasFile('image')) {
                $file = $request->image;
                $filename = time() . '.' . $file->getClientOriginalExtension();
                // $path = Storage::put('public/teams', $file, 'public');
                $path = $file->storeAs('public/announcement', $filename);
            ...
            if ($request->hasFile('bg_image')) {
                ...
                // $path = Storage::put('public/teams', $file, 'public');
                $path = $file->storeAs('public/announcement', $filename);
                $announcement->bg_image = $filename;
            }
            $announcement->save();
